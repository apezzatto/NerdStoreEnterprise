        ...
        private async Task<bool> ValidarCarrinhoProdutos(CarrinhoDTO carrinho, IEnumerable<ItemProdutoDTO> produtos)
        {
            if (carrinho.Itens.Count != produtos.Count())
            {
                ...
                foreach (var itemId in itensIndisponiveis)
                {
                    ...
                    AddError($"O item {itemCarrinho.Nome} não está mais disponível no catálogo, o remova do carrinho para prosseguir com a compra");
                }
            ...
            foreach (var itemCarrinho in carrinho.Itens)
            {
                ...
                if (produtoCatalogo.Valor != itemCarrinho.Valor)
                {
                    ...
                    {
                        AddError($"Não foi possível remover automaticamente o produto {itemCarrinho.Nome} do seu carrinho, _" +
                    ...
                    {
                        AddError($"Não foi possível atualizar automaticamente o produto {itemCarrinho.Nome} do seu carrinho, _" +
